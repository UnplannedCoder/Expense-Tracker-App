...
const callOpenRouter = async (messages) => {
  ...
  if (!apiKey) {
    throw new Error(
      "OPENROUTER_API_KEY is not set in environment variables. Add it to server/.env",
    );
  }
        ...
        new Error(
          JSON.stringify({
            error: {
              code: errCode,
              message: errMsg,
              status: data?.error?.type || String(res.status),
            },
          }),
        ),
        { status: res.status },
    ...
    const req = https.request(options, (res) => {
      ...
      res.on("end", () => {
        try {
          ...
          reject(
            new Error(
              `Failed to parse OpenRouter response: ${raw.substring(0, 200)}`,
            ),
          );
        }
      });
...
const parseError = (error) => {
  ...
  if (
    ...
    return new Error(
      "FinBot setup error: Invalid OpenRouter API key. Please check OPENROUTER_API_KEY in server/.env",
    );
  }
  if (
    code === 429 ||
    ...
    return new Error(
      "FinBot is temporarily busy. Please try again in a moment.",
    );
  }
  if (
    code === 402 ||
    ...
    return new Error(
      "FinBot service error: OpenRouter credits exhausted. Please top up at openrouter.ai",
    );
  }
  if (message.includes("SAFETY")) {
    return new Error(
      "I cannot respond to that request. Please ask me about your finances.",
    );
  }
  ...
  return new Error(
    isDev
      ? `FinBot error (${code}): ${message.substring(0, 150)}`
      : "FinBot encountered an error. Please try again.",
  );
};
