...
const fetchJSON = async (url, options = {}) => {
  ...
  try {
    ...
    throw new Error(`Bad JSON from ${url}: ${r.text.substring(0, 100)}`);
  }
};
...
const acquireYFCrumb = async () => {
  ...
  try {
    ...
    if (process.env.NODE_ENV !== "production") {
      console.debug(
        "[MarketData] YF crumb not available, using chart API fallback",
      );
    }
  }
  return false;
...
const getQuote = async (symbol) => {
  ...
  const quote = {
    ...
    regularMarketDayHigh:
      ...
      Math.max(...(ohlcv.high || [0]).filter(Boolean)),
    regularMarketDayLow:
      meta.regularMarketDayLow ||
      Math.min(...(ohlcv.low || [Infinity]).filter(Boolean)),
...
const getCryptoPrices = async (coins) => {
  const ids = [
    ...new Set(
      coins.map((c) => COINGECKO_IDS[c.toLowerCase()] || c.toLowerCase()),
    ),
  ].join(",");
  ...
  try {
    ...
    if (q?.regularMarketPrice) {
      const result = {
        ...
        rates: { INR: q.regularMarketPrice },
      };
      toCache(cacheKey, result);
      return result;
    }
...
const OVERVIEW_SYMBOLS = [
  "^NSEI",
  "^BSESN", // Indian indices (most important)
  "^GSPC",
  "^IXIC", // US indices
  "GC=F",
  "CL=F", // Gold, Crude Oil (Silver on-demand)
  "BTC-USD", // Bitcoin
  "USDINR=X", // Forex
];
...
const buildMarketContext = async (userMessage) => {
    ...
    fetches.push(
      (async () => {
        try {
          ...
          if (commodSymbols.length === 0)
            commodSymbols.push("GC=F", "SI=F", "CL=F");
          ...
          if (commodQuotes.length > 0) {
            ...
            lines.push(
              `  USD/INR used for conversion: ₹${Number(usdInr).toFixed(2)}`,
            );
            lines.push("");
            ...
            if (goldQ?.regularMarketPrice) {
              ...
              lines.push(
                `    Futures price: $${usdPerOz.toFixed(2)}/troy oz  ${arrow} ${sign}$${goldQ.regularMarketChange?.toFixed(2)} (${sign}${goldQ.regularMarketChangePercent?.toFixed(2)}%)`,
              );
              lines.push(
                `    Day range: $${goldQ.regularMarketDayLow} – $${goldQ.regularMarketDayHigh}`,
              );
              lines.push("");
              lines.push("    INDIA GOLD PRICES (INR):");
              lines.push(
                `    • 24 Karat (999 / pure gold): ₹${per1g24.toLocaleString("en-IN")} per gram  |  ₹${per10g24.toLocaleString("en-IN")} per 10g`,
              );
              lines.push(
                `    • 22 Karat (916 / jewellery): ₹${per1g22.toLocaleString("en-IN")} per gram  |  ₹${per10g22.toLocaleString("en-IN")} per 10g`,
              );
              lines.push("");
              hasData = true;
            }
            ...
            if (silverQ?.regularMarketPrice) {
              ...
              lines.push(
                `    Futures price: $${usdPerOz.toFixed(2)}/troy oz  ${arrow} ${sign}$${silverQ.regularMarketChange?.toFixed(2)} (${sign}${silverQ.regularMarketChangePercent?.toFixed(2)}%)`,
              );
              lines.push(
                `    Day range: $${silverQ.regularMarketDayLow} – $${silverQ.regularMarketDayHigh}`,
              );
              lines.push("");
            ...
            if (oilQ?.regularMarketPrice) {
              ...
              lines.push(
                `    $${usdPerBarrel.toFixed(2)}/barrel  ${arrow} ${sign}$${oilQ.regularMarketChange?.toFixed(2)} (${sign}${oilQ.regularMarketChangePercent?.toFixed(2)}%)`,
              );
              lines.push(
                `    ≈ ₹${inrPerBarrel.toLocaleString("en-IN")}/barrel`,
              );
              lines.push(
                `    Day range: $${oilQ.regularMarketDayLow} – $${oilQ.regularMarketDayHigh}`,
              );
              lines.push("");
              hasData = true;
            }
            ...
            if (wantsHistory && commodSymbols.length > 0) {
              const period =
                lower.includes("1 year") || lower.includes("yearly")
                  ? "1y"
                  : lower.includes("6 month")
                    ...
                    : lower.includes("3 month")
                      ...
                      : lower.includes("yesterday")
                        ? "5d"
                        : "1mo";
              const histSym = wantsGold
                ? "GC=F"
                : wantsSilver
                  ? "SI=F"
                  : "CL=F";
              const hist = await getHistoricalData(histSym, period, "1d");
              const slice = lower.includes("yesterday")
                ? hist.history.slice(-2)
                : hist.history.slice(-7);
              if (slice.length > 0) {
                const label =
                  histSym === "GC=F"
                    ? "Gold"
                    : histSym === "SI=F"
                      ...
                      : "Crude Oil";
                lines.push(
                  `  --- ${label} Futures Historical (${period}, last ${slice.length} sessions, USD/troy oz) ---`,
                );
                slice.forEach((d) =>
                  lines.push(
    ...
    fetches.push(
      (async () => {
        try {
          ...
          if (wantsHistory) {
            const period = lower.includes("1 year")
              ? "1y"
              : lower.includes("6 month")
                ? "6mo"
                : lower.includes("3 month")
            ...
            if (slice.length > 0) {
              lines.push(
                `--- Historical (${period}, last ${slice.length} sessions) ---`,
              );
              slice.forEach((d) =>
                lines.push(
                  `  ${d.date}: Open ${d.open} | High ${d.high} | Low ${d.low} | Close ${d.close}`,
      ...
      (async () => {
        try {
          ...
          if (lower.includes("bitcoin") || lower.includes("btc"))
            cryptoList.push("bitcoin");
          if (lower.includes("ethereum") || lower.includes("eth"))
            cryptoList.push("ethereum");
          if (lower.includes("bnb") || lower.includes("binance"))
            cryptoList.push("binancecoin");
          if (lower.includes("solana") || lower.includes("sol"))
            cryptoList.push("solana");
          if (lower.includes("dogecoin") || lower.includes("doge"))
            cryptoList.push("dogecoin");
          if (lower.includes("xrp") || lower.includes("ripple"))
            cryptoList.push("ripple");
          if (lower.includes("cardano") || lower.includes("ada"))
            cryptoList.push("cardano");
          if (lower.includes("polygon") || lower.includes("matic"))
            cryptoList.push("matic-network");
          if (cryptoList.length === 0)
            cryptoList.push("bitcoin", "ethereum", "binancecoin", "solana");
          ...
          Object.entries(prices).forEach(([id, d]) => {
            ...
            lines.push(
              `  ${id}: ₹${d.inr?.toLocaleString("en-IN")} | $${d.usd?.toLocaleString("en-US")}  ${arrow} ${sign}${change}% (24h)`,
            );
          });
          lines.push("");
          hasData = true;
          ...
          if (wantsHistory && cryptoList.length > 0) {
            const days = lower.includes("1 year")
              ? 365
              : lower.includes("6 month")
                ? 180
                : lower.includes("3 month")
            ...
            lines.push(
              `--- ${cryptoList[0]} INR Price History (last ${slice.length} days) ---`,
            );
            slice.forEach((d) =>
              lines.push(`  ${d.date}: ₹${d.priceINR.toLocaleString("en-IN")}`),
            );
            lines.push("");
          }
        } catch (e) {
        ...
        try {
          ...
          if (forex) {
            ...
            Object.entries(forex.rates).forEach(([cur, rate]) => {
              if (rate)
                lines.push(`  1 USD = ${cur} ${Number(rate).toFixed(2)}`);
            });
  ...
  if (
    wantsOverview ||
    (!specificTicker && !wantsCrypto && !wantsForex && !wantsCommodity)
  ) {
    fetches.push(
      (async () => {
        try {
  ...
  if (!hasData) {
    console.warn(
      "[MarketData] No data fetched for:",
      userMessage,
      "| Errors:",
      errors,
    );
  ...
  lines.push(
    "⚠ Disclaimer: Prices may be delayed ~15 min for exchange-listed instruments. This is not investment advice.",
  );
