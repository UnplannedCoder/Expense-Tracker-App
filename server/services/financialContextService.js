...
const buildFinancialContext = async (userId) => {
  try {
    ...
    const [currentMonthTx, lastMonthTx, recentTx, budgets, allTimeTx] =
      await Promise.all([
        Transaction.find({
          userId,
    ...
    currentMonthTx.forEach((tx) => {
      if (tx.type === "income") {
        ...
        currentCategories[tx.category] =
          (currentCategories[tx.category] || 0) + tx.amount;
      }
    });
    ...
    if (sortedCategories.length > 0) {
      ...
      sortedCategories.forEach(([cat, amt]) => {
        const pct =
          currentExpense > 0 ? Math.round((amt / currentExpense) * 100) : 0;
