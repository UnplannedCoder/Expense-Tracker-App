...
const Card = ({ label, value, sub, Icon, bg, color }) => (
  <div className="bg-white dark:bg-slate-900 border border-slate-300 dark:border-slate-800 p-4 sm:p-5 rounded-2xl shadow-sm">
    ...
    <p
      className={`text-lg sm:text-2xl font-extrabold leading-tight truncate ${color}`}
    >
      {value}
    </p>
...
const Analytics = () => {
  ...
  const savingsRate =
    monthly?.totalIncome > 0
      ? Math.round((monthly.netSavings / monthly.totalIncome) * 100)
      : 0;

  const highestMonth = areaData.reduce(
    (best, d) => (d.expense > (best?.expense || 0) ? d : best),
    null,
  );
  const bestSavingsMonth = areaData.reduce(
    (best, d) => (d.savings > (best?.savings || -Infinity) ? d : best),
    null,
  );
    ...
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3 sm:gap-4">
        <Card
          ...
          value={formatMoney(monthly?.totalIncome || 0, currency)}
        ...
        <Card
          ...
          value={formatMoney(monthly?.totalExpense || 0, currency)}
          sub={`${MONTHS[selMonth - 1]} ${selYear}`}
          Icon={FaArrowDown}
          bg="bg-rose-500/10"
        ...
        <Card
          ...
          value={formatMoney(yearly?.totalExpense || 0, currency)}
          sub={`Full year ${selYear}`}
          Icon={FaFire}
          bg="bg-amber-500/10"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-3 sm:gap-4">
        <div className="bg-indigo-50 dark:bg-indigo-950/30 border border-indigo-100 dark:border-indigo-900/40 rounded-2xl p-4 sm:p-5">
          ...
          <p
            className={`text-lg sm:text-xl font-extrabold truncate ${(yearly?.netSavings || 0) >= 0 ? "text-indigo-600 dark:text-indigo-400" : "text-rose-600"}`}
          >
            {formatMoney(yearly?.netSavings || 0, currency)}
          </p>
          <p className="text-xs text-slate-400 mt-1">
            Income minus all expenses for {selYear}
          </p>
        </div>
        <div className="bg-rose-50 dark:bg-rose-950/30 border border-rose-100 dark:border-rose-900/40 rounded-2xl p-4 sm:p-5">
          ...
          <p className="text-xs text-slate-400 mt-1">
            {highestMonth
              ? formatMoney(highestMonth.expense, currency)
              : "No data"}
          </p>
        </div>
        <div className="bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-100 dark:border-emerald-900/40 rounded-2xl p-4 sm:p-5">
          ...
          <p className="text-lg sm:text-xl font-extrabold text-emerald-600 dark:text-emerald-400">
            {bestSavingsMonth?.name || "—"}
          </p>
          <p className="text-xs text-slate-400 mt-1">
            {bestSavingsMonth
              ? formatMoney(bestSavingsMonth.savings, currency)
              : "No data"}
          </p>
        </div>
      </div>
