...
const clampToNow = (month, year) => {
  const now = new Date();
  if (year > now.getFullYear()) return { month: now.getMonth() + 1, year: now.getFullYear() };
...
const Dashboard = () => {
  ...
  const [metrics, setMetrics] = useState({
    ...
    categoryBreakdown: [], month: null, year: null,
  });
  ...
  const barChartData = [{ name: 'Month', Income: metrics.totalIncome, Expense: metrics.totalExpense }];
